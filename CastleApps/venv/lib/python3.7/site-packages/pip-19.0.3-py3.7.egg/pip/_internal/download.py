...
class MultiDomainBasicAuth(AuthBase):
    ...
    def warn_on_401(self, resp, **kwargs):
        # warn users that they provided incorrect credentials
        if resp.status_code == 401:
...
class SafeFileCache(FileCache):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # is owned by the users current executing pip. If it does not exist
        # we will check the parent directory until we find one that does exist.
        # If it is not owned by the users executing pip then we will disable
        # the cache and log a warning.
        if not check_path_owner(self.directory):
            logger.warning(
                "The directory '%s' or its parent directory is not owned by "
                "the current users and the cache has been disabled. Please "
...
def _copy_dist_from_dir(link_path, location):
    ...
    Invoked when users requests to install a local directory. E.g.:
